...
export default function Popup(): JSX.Element {
  ...
  const [barContentList, setBarContentList] = useState<BarContent[]>([
    {
      ...
      action:
        "Exit this email immediately and report the email to your supervisor or IT department.",
      color: "red",
    },
    {
      suspiciousPortion: "support@goegle.com",
      reason:
        "Seems suspicious at level 6 because it may be trying to confuser users with google.com.",
      action:
        "Look into this domain, and make sure it is truly somebody you would receive an email from.",
      color: "yellow",
    },
    {
      suspiciousPortion: "support@google.com",
      reason:
        "Nothing looks out of place. The email address, subject line, and content aren't suspicious.",
  ...
  useEffect(() => {
    const receive = (message: any, _: any, _a: any) => {
      if (message.action === "receiveInference") {
        ...
        <div className="flex gap-2 p-3 rounded-sm bg-neutral-200">
          {barContentList.map((barContent: BarContent, index: number) => {
            return (
              <div
                key={index}
                className={`w-3 h-8 rounded-sm ${getBarColor(
                  barContent.color
                )}`}
